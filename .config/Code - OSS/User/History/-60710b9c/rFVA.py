...
def solve_mean_estimate():
    omega_average_estimate_array = np.zeros(len(task1.SNRs_in_dB))
    phi_average_estimate_array = np.zeros(len(task1.SNRs_in_dB))
    for i, fft_size in enumerate(task1.FFT_sizes):
        for j, SNR_dB in enumerate(task1.SNRs_in_dB):
            ...
            omega_variance_array[j] = omega_variance
            omega_average_estimate_array[j] = np.mean(omega_estimates)
            phi_variance_array[j] = phi_variance
            phi_average_estimate_array[j] = np.mean(phi_estimates)
