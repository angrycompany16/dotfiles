...
type UI struct {
	...
	activeOverlays []*overlay.Overlay
}
...
func (ui *UI) Update() {
	...
	for _, overlay := range ui.activeOverlays {
		if overlay.
	}
}
...
func (ui *UI) AddOverlay(overlay *overlay.Overlay) {
	ui.activeOverlays = append(ui.activeOverlays, overlay)
	overlay.StartFadeIn()
}

func (ui *UI) StartOverlay(name string) {
	if overlay, ok := ui.overlays[name]; ok {
		overlay.StartFadeIn()
	}
}
...
func NewUI() *UI {
	return &UI{
		// ActiveOverlay: overlay.NewOverlay(overlay.NewScreenFade()),
