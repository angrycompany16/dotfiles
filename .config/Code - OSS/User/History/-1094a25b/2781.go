...
func (w *World) Load(loadedMemory map[string]levelmemory.LevelMemory) {
	w.worldLDTK = assettypes.NewLDTKAsset(LDTKMapPath)

	for id, levelmemory := range loadedMemory {
...
func ChangeActiveLevel[T string | int](world *World, id T, doorIid string) error {
	...
	if memory, ok := world.worldStateMemory[newLevelLDTK.Iid]; ok {
		// fmt.Println("Restoring level")
		newLevel.restoreFromMemory(&memory)
	}

	if world.ActiveLevel != nil {
		world.worldStateMemory[world.ActiveLevel.levelLDTK.Iid] = levelmemory.LevelMemory{world.ActiveLevel.GetSlamboxPositions()}
	}
	world.worldStateMemory[newLevel.levelLDTK.Iid] = levelmemory.LevelMemory{newLevel.GetSlamboxPositions()}
...
func (w *World) ResetActiveLevel() (float64, float64) {
	...
	// w.ActiveLevel.slamboxes = w.worldStateMemory[w.ActiveLevel.levelLDTK.Iid].SlamboxPositions
