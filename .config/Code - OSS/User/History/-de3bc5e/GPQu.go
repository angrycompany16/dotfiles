...
func (p *Particle) draw(layer *ebiten.Image, offsetX, offsetY float64) {
	...
	// ebitenrenderutil.DrawAt(image, layer, p.posX-offsetX, p.posY-offsetY)
	// fmt.Println(p.posX-offsetX, p.posY-offsetY)

	vector.DrawFilledCircle(layer, float32(p.posX-offsetX), float32(p.posY-offsetY), 5, color.Black, false)
