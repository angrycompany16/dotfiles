...
func (g *Game) updateGameplay() error {
	...
	if g.player.GetLevelSwapInput() && doorOverlap && !g.player.Disabled {
		biomeSwitch := errs.Must(world.ChangeActiveLevel(g.world, levelIid, doorEntityIid))
		if biomeSwitch {
			titleCard := g.gameUI.GetOverlay("titlecard")
			titleCard.OverlayContent.(*overlay.TitleCard)
		}
	...
	if wasHit && !g.player.Disabled || restartPrompted {
		g.player.Die()
		g.gameUI.ActiveOverlay.StartFadeIn()
	}
	...
	if raised {
		...
		g.gameUI.ActiveOverlay.StartFadeOut()
	}

	g.player.Update()
	g.gameUI.ActiveOverlay.Update()
...
func NewGame() *Game {
	...
	game.deathEffectEnterListener = events.NewEventListener(game.gameUI.ActiveOverlay.OnFinishEnter)
