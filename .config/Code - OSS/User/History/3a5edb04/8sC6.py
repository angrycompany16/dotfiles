...
def plot_average_omega_against_snr(omega_averages: np.typing.ArrayLike):
    ...
    plt.plot(task1.SNRs_in_dB, omega_averages, marker='o', linestyle='-', label=f'Omega Average Estimate')

    plt.plot(task1.SNRs_in_dB, true_value, label=f'True value of omega')

    plt.yscale('log')
    plt.xlabel('SNR (dB)')
    plt.ylabel('Variance')
    plt.title('Omega Estimate Variance and CRLB vs SNR')
...
def plot_average_phi_against_snr(phi_averages: np.typing.ArrayLike):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(task1.SNRs_in_dB, phi_averages, marker='o', linestyle='-', label=f'Phi Average Estimate')

    plt.plot(task1.SNRs_in_dB, true_value, label=f'True value of phi')
