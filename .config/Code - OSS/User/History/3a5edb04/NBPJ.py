...
def compute_omega_and_phi_estimates(SNR_dB: float, FFT_size: float):
    ...
    omega_estimates_rough = np.zeros(task1.iterations)
    ...
    for n in range(task1.iterations) :
        ...
        omega_est = 2 * np.pi * m_max / (FFT_size * task1.T)
        omega_estimates_rough[n] = omega_est
        
        
        phi_est = task1.get_phi_est(omega_est, x_samples)
