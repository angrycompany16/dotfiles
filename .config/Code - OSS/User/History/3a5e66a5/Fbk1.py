...
FFT_sizes = [np.pow(2, k) for k in range(10, 23, 2)]
...
def plot_average_omega_against_snr(omega_averages_array: np.typing.ArrayLike):
    ...
    for i, omega_averages in enumerate(omega_averages_array):
        plt.plot(SNRs_in_dB, omega_averages, marker='o', linestyle='-', label=f'Average Frequency Estimate  w/ fft size 2^{i}')
    ...
    plt.title('Average Frequency Estiamte and True Value vs SNR')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def plot_average_phi_against_snr(phi_averages_array: np.typing.ArrayLike):
    ...
    for i, phi_averages in enumerate(phi_averages_array):
        plt.plot(SNRs_in_dB, phi_averages, marker='o', linestyle='-', label=f'Average Phase Estimate w/ fft_size 2^{i}')
