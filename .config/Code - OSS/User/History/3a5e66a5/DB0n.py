...
def plot_variances_omega_against_snr(omega_variances: np.typing.ArrayLike):
    ...
    plt.plot(SNRs_in_dB, clipped_omega_variances, marker='o', linestyle='-', label='Omega Variance')
    plt.plot(SNRs_in_dB, omega_CRLBs, marker='s', linestyle='--', label='Omega CRLB')
...
def plot_average_omega_against_snr(omega_averages_array: np.typing.ArrayLike):
    true_value = np.full(len(FFT_sizes), omega_0)

    plt.figure(figsize=(10, 6))
    for i, omega_averages in enumerate(omega_averages_array):
        plt.plot(SNRs_in_dB, omega_averages, marker='o', linestyle='-', label=f'Omega Variance w/ fft size {i}')

    plt.plot(SNRs_in_dB, true_value, label=f'Omega Variance w/ fft size {i}')
...
def plot_average_phi_against_snr(phi_averages_array: np.typing.ArrayLike):
    true_value = np.full(len(FFT_sizes), phi)
    plt.figure(figsize=(10, 6))
